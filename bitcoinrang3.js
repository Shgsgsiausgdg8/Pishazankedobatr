...
class TradingStrategy {
    ...
    async detectFib382And618WithDivergence(candles, currentPrice, timeframe, multiTimeframeData = null) {
...
class BitcoinEngine {
    ...
    async start() {
        ...
        setInterval(() => {
            if (this.isEnabled && this.price > 0) {
        ...
        setInterval(async () => {
            if (this.isEnabled) {
                await this.multiTimeframeManager.updateAllTimeframes();
                console.log("🔄 داده‌های تایم فریم‌های بیت‌کوین به‌روز شد");
            }
        }, 60 * 1000);
...
const engine = new BitcoinEngine();
engine.start();

setInterval(async () => {
    if (engine.isEnabled && engine.candles.length > 0) {
        await engine.runStrategy();
    }
}, 60 * 1000);

setInterval(() => {
    if (engine.isEnabled && engine.price > 0) {
        const state = engine.getState();
        console.log(`📊 [${new Date().toLocaleTimeString('fa-IR')}] قیمت بیت‌کوین: ${state.price.toLocaleString()} USD | سیگنال: ${state.signalsCount} | معاملات فعال: ${state.activeTradesCount} | معامله بعدی: ${state.nextTradeNumber} | bit22`);
    }
}, 60 * 1000);

process.on('SIGINT', () => {
    console.log("\n🛑 در حال توقف ربات بیت‌کوین... bit22");
    engine.saveActiveTrades();
    engine.saveTradeStats();
    engine.saveSettings();
    if (engine.ws) {
        engine.ws.terminate();
    }
